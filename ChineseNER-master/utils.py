...
def test_ner(results, path):
    ...
    Run perl script to evaluate model
    """
    output_file = os.path.join(path, "voice_predict.utf8")
...
def result_to_json1(string, tags):
...
def result_to_json(string, tags):
    item = {"user_says": string, "slots": result_to_json2(string, tags)}
    return item

def result_to_json2(string, tags):
    print("@@@@@@@@@@@@@@@@@@@@@@@",tags)
    newB_flag = 'O'
    cnt = 0
    result = {}
    for tag in tags:
        newB_flag = tag[0]
        if newB_flag == 'B':
            result[''.join(tag[2:])] = string[cnt]
        if newB_flag == 'I':
            result[''.join(tag[2:])] = result[''.join(tag[2:])] + string[cnt]
        cnt = cnt + 1
    tmp = {}
    for key in result.keys():
        tmp[key] = {'type': key, 'value': result[key]}
    return tmp
